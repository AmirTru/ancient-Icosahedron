...
uniform vec3 uLightAccentColor;
...
vec3 getDisplacedPosition(vec3 _position) {
    ...
    distoredPosition += perlin4d(vec4(distoredPosition * uDistortionFrequency + uOffset, uTime * uSpeed)) * uDistortionStrength;

    float perlinStrength = perlin4d(vec4(distoredPosition * uDisplacementFrequency + uOffset, uTime * uSpeed));
...
void main() {
    ...
    vec3 scaledPosition = displacedPosition * uSize;
    vec4 viewPosition = viewMatrix * vec4(scaledPosition, 1.0);
